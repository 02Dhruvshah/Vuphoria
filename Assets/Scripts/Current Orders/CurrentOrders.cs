...
{
    public string currentOrdersURL = "http://172.21.0.90/SQLData.php?Command=currentOrders";
    public CurrentOrderJSON[] currentOrderObjectArray;
    ...
    {
        Debug.Log("Updated Current Order");
       if (listOfOrderOptionsPanel.transform.childCount < 1)
        {
            ...
            {
                StartCoroutine(GetRequest(currentOrdersURL));
    ...
    public void ConvertOrderDataToArray(string OrderStringPHPMany)
    {
        ...
        currentOrderObjectArray = JsonHelper.FromJson<CurrentOrderJSON>(severCodeFixOrderStringPHPMany);
        Debug.Log("Converting ONo:" + currentOrderObjectArray[0].ONo + ", Company:" + currentOrderObjectArray[0].Company + ", PlannedStart:" + currentOrderObjectArray[0].PlannedStart + ", PlannedEnd:" + currentOrderObjectArray[0].PlannedEnd + ", State:" + currentOrderObjectArray[0].State);
        for (int i = 0; i < currentOrderObjectArray.Length; i++)
        {
            Debug.Log("ONo:" + currentOrderObjectArray[i].ONo + ", Company:" + currentOrderObjectArray[i].Company + ", PlannedStart:" + currentOrderObjectArray[i].PlannedStart + ", PlannedEnd:" + currentOrderObjectArray[i].PlannedEnd + ", State:" + currentOrderObjectArray[i].State);
        }
    ...
    public void UpdateOrderData()
    {
        ...
        for (int i = 0; i < currentOrderObjectArray.Length; i++)
        {
            ...
            CurrentOrderDisplay OrderButtonScript = InstantiatedOrderButton.GetComponent(typeof(CurrentOrderDisplay)) as CurrentOrderDisplay;
            OrderButtonScript.ONo.text =  "ONo: " + currentOrderObjectArray[i].ONo.ToString();
            OrderButtonScript.Company.text =  "Company: " + currentOrderObjectArray[i].Company;
            OrderButtonScript.PlannedStart.text = "PlannedStart: " + currentOrderObjectArray[i].PlannedStart;
            OrderButtonScript.PlannedEnd.text ="PlannedEnd: " + currentOrderObjectArray[i].PlannedEnd;
